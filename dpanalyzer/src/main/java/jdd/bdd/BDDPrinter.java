...
public class BDDPrinter {
	...
	final private static List<Integer> HEADER_FIELDS = new ArrayList<>(List.of(31, 63, 79, 95));
	...
	private static void printSet_rec(int bdd, int level) {
		...
		if(level == set_chars_len) {
			BDDPrinter.seqCount+=1;
			
			if (BDDPrinter.seqCount <= 10) {
				if(nn == null) {
					// JDDConsole.out.print("_");
					for(int i = 0; i < set_chars_len; i++) {
						JDDConsole.out.print(set_chars[i]);
						if (HEADER_FIELDS.contains(i)) {
							JDDConsole.out.print("_");
